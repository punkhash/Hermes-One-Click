#!/usr/bin/env python3
"""One-shot bootstrap launcher for Cosmius Hermes."""
...
def install_hermes_agent() -> None:
    info(f"Cosmius Hermes Agent not found. Attempting install via {INSTALLER_URL}")
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap Cosmius Hermes onboarding.")
...
def main() -> int:
    ...
    if not agent_dir and not hermes_command_exists():
        if args.skip_agent_install:
            raise RuntimeError(
                "Cosmius Hermes Agent was not found and auto-install was disabled."
            )
    ...
    info(f"Starting Cosmius Hermes on http://{args.host}:{args.port}")
