...
from api.providers import (
    _provider_runtime_env_updates,
    _write_env_file,
    sync_install_env_file,
)  # shared impl with _ENV_LOCK (#1164)
...
_SUPPORTED_PROVIDER_SETUPS = {
    ...
    "custom": {
        ...
        "default_base_url": "https://token.kyvolen.com/v1",
        "requires_base_url": True,
        "models": [],
        "category": "self_hosted",
        "quick": True,
    ...
    "deepseek": {
        ...
        "requires_base_url": False,
        # Public DeepSeek API uses one global host; /v1 is for OpenAI-compatible clients.
        "base_url_presets": [
            {
                "id": "official",
                "url": "https://api.deepseek.com",
                "label_key": "onboarding_deepseek_preset_default",
            },
            {
                "id": "openai_v1",
                "url": "https://api.deepseek.com/v1",
                "label_key": "onboarding_deepseek_preset_openai_v1",
            },
        ],
...
def _quick_setup_env_vars() -> set[str]:
    env_vars = {
        str(meta.get("env_var") or "").strip()
        for meta in _SUPPORTED_PROVIDER_SETUPS.values()
    }
    # Alias keys written/read by the packaged Kyvolen and Gemini flows.
    env_vars.update({"KYVOLEN_API_KEY", "GEMINI_API_KEY", "LM_API_KEY"})
    return {env_var for env_var in env_vars if env_var}


def _reset_quick_setup_env_files() -> None:
    env_vars = _quick_setup_env_vars()
    updates = _provider_runtime_env_updates(
    ...
    try:
        for env_var in sorted(env_vars):
            sync_install_env_file(env_var=env_var, api_key=None)
        sync_install_env_file(base_url=DEFAULT_BASE_URL, default_model=DEFAULT_MODEL)
    except Exception:
        logger.debug("reset install env file sync failed", exc_info=True)


def _reset_quick_setup_config(cfg: dict) -> dict:
    ...
    if isinstance(providers_cfg, dict):
        provider_ids = set(_SUPPORTED_PROVIDER_SETUPS.keys())
        provider_ids.update({"google", "xai", "lmstudio"})
...
def _build_setup_catalog(cfg: dict, force_default: bool = False) -> dict:
    if force_default:
        current_provider = "custom"
        current_model = DEFAULT_MODEL
        current_base_url = _SUPPORTED_PROVIDER_SETUPS["custom"]["default_base_url"]
    else:
        current_provider = _extract_current_provider(cfg) or "custom"
        current_model = _extract_current_model(cfg)
        current_base_url = _extract_current_base_url(cfg)
        if current_provider == "custom" and not current_base_url:
            current_base_url = _SUPPORTED_PROVIDER_SETUPS["custom"]["default_base_url"]
        ...
        providers.append(
            {
                ...
                "base_url_presets": list(meta.get("base_url_presets") or []),
...
def get_onboarding_status() -> dict:
    ...
    force_wizard = bool(settings.get("onboarding_force_wizard"))
    ...
    config_auto_completed = (not force_wizard) and config_exists and (
    ...
    return {
        "completed": (
            auto_completed
            or (bool(settings.get("onboarding_completed")) and not force_wizard)
        ...
        "settings": {
            ...
            "bot_name": settings.get("bot_name") or "Cosmius Hermes",
        },
        "system": {
        ...
        "setup": _build_setup_catalog(cfg, force_default=force_wizard),
        "workspaces": {
            "items": workspaces,
            "last": last_workspace,
        },
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if skip_env in {"1", "true", "yes"}:
        save_settings({"onboarding_completed": True, "onboarding_force_wizard": False})
    ...
    if provider_meta.get("requires_base_url"):
        ...
        # Honor user-edited base URL from the wizard (e.g. DeepSeek /v1 path).
        model_cfg["base_url"] = base_url or provider_meta["default_base_url"]
    else:
    ...
    if api_key:
        _write_env_file(
            ...
            _provider_runtime_env_updates(
                ...
                default_model=model_cfg.get("default") or DEFAULT_MODEL,
            ),
        )
        try:
            from api.providers import sync_install_env_file
            ...
            logger.debug("Failed to sync install .env after onboarding setup", exc_info=True)
    ...
    try:
        ...
        _routes._clear_live_models_cache()
    except Exception:
        logger.debug("clear live models cache after onboarding setup failed", exc_info=True)
    return get_onboarding_status()
...
def reset_to_initial_setup(body: dict) -> dict:
    """Reset quick setup state and reopen the first-run wizard."""
    ...
    if skip_env in {"1", "true", "yes"}:
        raise RuntimeError(
            "Reset is disabled: HERMES_WEBUI_SKIP_ONBOARDING is set for this instance."
        )
    ...
    try:
        config_path = _get_config_path()
        cfg = _load_yaml_config(config_path)
        cfg = _reset_quick_setup_config(cfg)
        _save_yaml_config(config_path, cfg)
        _reset_quick_setup_env_files()
        reload_config()
    except Exception:
        logger.debug("reset onboarding default provider config failed", exc_info=True)
    try:
        from api.profiles import _reload_dotenv
        _reload_dotenv(_get_active_hermes_home())
    except Exception:
        logger.debug("Failed to reload dotenv after onboarding reset", exc_info=True)
    try:
        from hermes_cli import config as _hcfg
        _cache = getattr(_hcfg, '_LOAD_CONFIG_CACHE', None)
        if _cache is not None:
        ...
        logger.debug("Failed to reload hermes_cli config after onboarding reset", exc_info=True)
    try:
        import api.routes as _routes

        _routes._clear_live_models_cache()
    except Exception:
        logger.debug("clear live models cache after onboarding reset failed", exc_info=True)

    save_settings(
        {
            "onboarding_completed": False,
            "onboarding_force_wizard": True,
