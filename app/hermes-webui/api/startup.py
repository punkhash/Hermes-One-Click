...
def _patch_skill_module_paths(home: Path) -> None:
    """Point imported skill modules at the active WebUI profile home."""
    try:
    ...
    try:
        ...
        _skills_sync.HERMES_HOME = home
        _skills_sync.SKILLS_DIR = home / 'skills'
        _skills_sync.MANIFEST_FILE = _skills_sync.SKILLS_DIR / '.bundled_manifest'
    except (ImportError, AttributeError):
        pass


def sync_bundled_skills(*, quiet: bool = True) -> bool:
    """Seed bundled agent skills into the active profile without overwriting edits."""
    home = _active_hermes_home()
    _patch_skill_module_paths(home)
    try:
        ...
        if not quiet:
            ...
            print(f'[ok] Bundled skills ready: {total} bundled, {copied} copied, {updated} updated.', flush=True)
        return True
    except ImportError as e:
        if not quiet:
            print(f'[!!] Bundled skills sync skipped: {e}', flush=True)
        return False
    except Exception as e:
        if not quiet:
            print(f'[!!] Bundled skills sync failed: {e}', flush=True)
        return False
