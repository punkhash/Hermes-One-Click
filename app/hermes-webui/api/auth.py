...
PUBLIC_PATHS = frozenset({
    ...
    '/api/desktop/heartbeat', '/api/desktop/heartbeat/status',
})
...
def clear_session_cookies_memory() -> None:
    """Drop all signed-in browser sessions and remove the persisted sessions file."""
    _sessions.clear()
    try:
        if _SESSIONS_FILE.exists():
            _SESSIONS_FILE.unlink()
    except OSError:
        logger.debug("Could not remove sessions file %s", _SESSIONS_FILE, exc_info=True)
