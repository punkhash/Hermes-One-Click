...
def _detect_webui_version() -> str:
    ...
    Resolution order:
      1. ``api/_version.py`` — a fallback written by the Docker / CI release
         workflow when ``.git`` is not present in the image.  Expected to define
         ``__version__ = 'vX.Y.Z'``.  Cosmius Hermes also writes this file for
         desktop builds so branded releases show the packaged app version even
    ...
    """
    # Desktop/baked-image version: parse with regex rather than exec() — the
    # file holds exactly one assignment and regex is sufficient.
...
_RAW_WEBUI_VERSION: str = _detect_webui_version()


def _build_webui_version_with_nonce(raw: str) -> str:
    """Append a per-process nonce when the resolved version cannot uniquely
    identify a build (``"unknown"`` or a ``-dirty`` working tree).

    Why we need this: Hermes ships as a desktop EXE that loads its UI from a
    local HTTP server.  All ``<script src="static/foo.js?v=__WEBUI_VERSION__">``
    tags inherit this string.  When we run from a non-git staging tree the raw
    value is always ``"unknown"`` — so every server restart serves identical
    URLs.  Edge / Chromium "App Window" mode treats those URLs as cache hits
    and silently keeps stale JS even after we patch the source, which broke
    multiple bug-fix attempts (left-sidebar-stuck-stale-cache).  By appending a
    millisecond-precision per-process nonce we make each process start a
    distinct cache key, forcing the browser to re-fetch every JS file on each
    EXE relaunch.

    Released builds (``vX.Y.Z`` from ``git describe``) keep their pristine
    version so production cache hits still work.
    """
    needs_nonce = (not raw) or raw == 'unknown' or raw.endswith('-dirty')
    if not needs_nonce:
        return raw
    nonce = f"{int(time.time() * 1000):x}"
    base = raw or 'unknown'
    return f"{base}-r{nonce}"


WEBUI_VERSION: str = _build_webui_version_with_nonce(_RAW_WEBUI_VERSION)
...
def _restart_exec_argv() -> list[str]:
    """Build argv for os.execv so self-restart survives Windows argv quirks.

    Using ``[sys.executable] + sys.argv`` breaks when ``sys.argv[0]`` duplicates
    the interpreter, when the shell split a path on spaces, or when argv[0] is
    otherwise not a plain ``server.py`` invocation. Prefer ``sys.orig_argv``
    (Python 3.10+) which mirrors the real process command line.
    """
    import sys
    from pathlib import Path
    ...
    try:
        exe_r = Path(exe).resolve()
        while argv:
            try:
                if Path(argv[0]).resolve() == exe_r:
                    argv = argv[1:]
                    continue
