...
def _is_kyvolen_base_url(base_url: str | None) -> bool:
    try:
...
def _get_install_env_path() -> Path | None:
    raw = os.getenv("HERMES_INSTALL_ENV_FILE", "").strip()
    if not raw:
        return None
    try:
...
def sync_install_env_file(
    *,
    ...
    """Mirror WebUI provider settings into the installation-root .env file."""
    env_path = _get_install_env_path()
    if env_path is None:
        return
    updates: dict[str, str | None] = {}
    if env_var:
        updates[env_var] = api_key
        if env_var == "OPENAI_API_KEY" and _is_kyvolen_base_url(base_url or DEFAULT_BASE_URL):
            updates["KYVOLEN_API_KEY"] = api_key
    if base_url is not None:
        normalized = str(base_url or DEFAULT_BASE_URL).strip().rstrip("/") or DEFAULT_BASE_URL
        updates["HERMES_WEBUI_DEFAULT_BASE_URL"] = normalized
        updates["OPENAI_BASE_URL"] = normalized
        updates["HERMES_OPENAI_BASE_URL"] = normalized
    if default_model is not None:
        updates["HERMES_WEBUI_DEFAULT_MODEL"] = str(default_model or DEFAULT_MODEL).strip() or DEFAULT_MODEL
    if updates:
        _write_env_file(env_path, updates)


def _provider_runtime_env_updates(
    *,
    env_var: str | None = None,
    api_key: str | None = None,
    base_url: str | None = None,
    ...
    updates: dict[str, str | None] = {}
    normalized_base_url = str(base_url or DEFAULT_BASE_URL).strip().rstrip("/") or DEFAULT_BASE_URL
    if env_var:
        updates[env_var] = api_key
        if env_var == "OPENAI_API_KEY" and _is_kyvolen_base_url(normalized_base_url):
            updates["KYVOLEN_API_KEY"] = api_key
    if base_url is not None:
        updates["HERMES_WEBUI_DEFAULT_BASE_URL"] = normalized_base_url
        updates["OPENAI_BASE_URL"] = normalized_base_url
        updates["HERMES_OPENAI_BASE_URL"] = normalized_base_url
    if default_model is not None:
        updates["HERMES_WEBUI_DEFAULT_MODEL"] = str(default_model or DEFAULT_MODEL).strip() or DEFAULT_MODEL
    return updates
...
def _provider_base_url(provider_id: str, cfg: dict[str, Any]) -> str:
    if provider_id != "custom":
        return ""
    model_cfg = cfg.get("model", {})
    if isinstance(model_cfg, dict):
        return str(model_cfg.get("base_url") or DEFAULT_BASE_URL).strip().rstrip("/")
    return DEFAULT_BASE_URL


def _provider_default_base_url(provider_id: str) -> str:
    return DEFAULT_BASE_URL if provider_id == "custom" else ""


def _save_provider_base_url(provider_id: str, base_url: str | None) -> None:
    if provider_id != "custom":
        return
    ...
    with _cfg_lock:
        ...
        model_cfg.setdefault("default", DEFAULT_MODEL)
        model_cfg["base_url"] = normalized
...
def get_providers() -> dict[str, Any]:
    ...
    for pid in sorted(known_ids):
        ...
        providers.append({
            ...
            "base_url": _provider_base_url(pid, cfg),
            "default_base_url": _provider_default_base_url(pid),
    ...
    *,
    ...
    if base_url is not None:
        try:
            ...
            logger.exception("Failed to save base_url for provider %s", provider_id)
            return {"ok": False, "error": f"Failed to save Base URL: {exc}"}

    # Invalidate the model cache so the dropdown refreshes on next request.
    # Base URL edits also refresh config so the next chat request uses the new endpoint.
    invalidate_models_cache()
    if base_url is not None:
        reload_config()
    ...
    return {
        ...
        "action": "updated" if (api_key or base_url is not None or not update_key) else "removed",
    }
