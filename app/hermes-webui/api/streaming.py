...
# Count of in-flight _run_agent_streaming workers (factory reset waits for zero
# before closing SessionDB / deleting Hermes home on Windows).
_STREAMING_WORKER_COUNT = 0
_STREAMING_WORKER_COUNT_LOCK = threading.Lock()
...
def wait_for_streaming_workers_idle(timeout: float = 30.0) -> bool:
    """Block until all agent stream workers have exited their outer ``finally``.

    Used before factory reset deletes ``state.db`` — cached agents keep SQLite
    open until workers finish and we explicitly ``close()`` their ``SessionDB``.
    """
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        with _STREAMING_WORKER_COUNT_LOCK:
            if _STREAMING_WORKER_COUNT <= 0:
                return True
        time.sleep(0.05)
    logger.warning("wait_for_streaming_workers_idle timed out after %ss", timeout)
    return False


def _close_agent_session_db_if_any(agent) -> None:
    ...
    try:
        ...
        logger.debug("SessionDB.close failed on factory reset / teardown", exc_info=True)
    try:
...
def close_all_agent_session_db_handles() -> None:
    """Close SQLite on every cached / tracked agent so ``state.db`` can be removed."""
    with STREAMS_LOCK:
        for agent in list(AGENT_INSTANCES.values()):
            _close_agent_session_db_if_any(agent)
    with SESSION_AGENT_CACHE_LOCK:
        for _entry in list(SESSION_AGENT_CACHE.values()):
            agent = _entry[0] if isinstance(_entry, tuple) and _entry else None
...
def _run_agent_streaming(
    ...
    *,
    ...
    """
    global _STREAMING_WORKER_COUNT
    ...
    _ckpt_thread = None
    _agent_lock = None
    _streaming_worker_counted = False
    try:
        s = get_session(session_id)
        with _STREAMING_WORKER_COUNT_LOCK:
            _STREAMING_WORKER_COUNT += 1
        _streaming_worker_counted = True
        ...
        _thread_env.setdefault("HERMES_API_TIMEOUT", os.getenv("HERMES_API_TIMEOUT", "180"))
        _thread_env.setdefault("HERMES_API_CALL_STALE_TIMEOUT", os.getenv("HERMES_API_CALL_STALE_TIMEOUT", "180"))
        _thread_env.setdefault("HERMES_DISABLE_LAZY_INSTALLS", os.getenv("HERMES_DISABLE_LAZY_INSTALLS", "1"))
        ...
        with _ENV_LOCK:
            ...
            os.environ.setdefault('HERMES_API_TIMEOUT', _thread_env['HERMES_API_TIMEOUT'])
            os.environ.setdefault('HERMES_API_CALL_STALE_TIMEOUT', _thread_env['HERMES_API_CALL_STALE_TIMEOUT'])
            os.environ.setdefault('HERMES_DISABLE_LAZY_INSTALLS', _thread_env['HERMES_DISABLE_LAZY_INSTALLS'])
            if _profile_home:
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            ...
            timeout = 600
            try:
                from api.config import get_config as _get_config_for_clarify
                _agent_cfg = _get_config_for_clarify().get("agent", {})
                if isinstance(_agent_cfg, dict):
                    timeout = int(_agent_cfg.get("clarify_timeout") or timeout)
            except Exception:
                timeout = 600
        ...
        try:
            ...
            if _cfg.get("toolsets") == ["hermes-cli"]:
                _toolsets = ["hermes-cli"]
        ...
        if s is not None:
            ...
            with _lock_ctx:
                ...
                if pending_user_message:
                    existing_messages = list(getattr(s, "messages", []) or [])
                    last_message = existing_messages[-1] if existing_messages else None
                    if not (
                        isinstance(last_message, dict)
                        and last_message.get("role") == "user"
                        and last_message.get("content") == pending_user_message
                    ):
                        s.messages.append({
                            'role': 'user',
