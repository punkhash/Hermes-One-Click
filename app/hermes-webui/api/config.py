...
def _discover_python(agent_dir: Path) -> str:
    ...
    Priority:
      ...
      4. System python3
    """
    env_py = os.getenv("HERMES_WEBUI_PYTHON", "").strip()
    if env_py:
        env_path = Path(env_py).expanduser()
        if env_path.is_file():
            return str(env_path)
...
DEFAULT_MODEL = os.getenv("HERMES_WEBUI_DEFAULT_MODEL", "deepseek-v4-flash").strip()
DEFAULT_BASE_URL = os.getenv("HERMES_WEBUI_DEFAULT_BASE_URL", "https://token.kyvolen.com/v1").strip().rstrip("/")
...
def print_startup_config() -> None:
    ...
    lines = [
        "",
        "  Cosmius Hermes Web UI -- startup config",
        "  --------------------------------",
    ...
    if not _HERMES_FOUND:
        print(
            f"{err}  Could not find the Cosmius Hermes agent directory.\n"
...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    if isinstance(model_cfg, dict):
        ...
        config_base_url = model_cfg.get("base_url") or DEFAULT_BASE_URL
    ...
    def _is_kyvolen_base_url(value: str | None) -> bool:
        try:
            host = (urlparse(str(value or "")).hostname or "").lower()
        except Exception:
            return False
        return host == "token.kyvolen.com"

    def _is_remote_product_base_url(value: str | None) -> bool:
        try:
            host = (urlparse(str(value or "")).hostname or "").lower()
        except Exception:
            return False
        return host in {"api.deepseek.com", "token.kyvolen.com"}

    if _is_kyvolen_base_url(config_base_url):
        config_provider = "custom"

    if not model_id:
        return model_id, config_provider, config_base_url

    def _is_legacy_local_default(value: str | None) -> bool:
        return str(value or "").strip().lower() in {"qwen3:8b", "qwen3", "llama3", "llama3.1"}

    if _is_legacy_local_default(model_id) and _is_remote_product_base_url(config_base_url):
        return DEFAULT_MODEL, config_provider, config_base_url
    ...
    if isinstance(custom_providers, list):
        for entry in custom_providers:
            ...
            if entry_model and entry_name and model_id == entry_model:
                if _is_loopback_base_url(entry_base_url) and _is_remote_product_base_url(config_base_url):
                    return DEFAULT_MODEL, config_provider, config_base_url
    ...
    if model_id.startswith("@") and ":" in model_id:
        ...
        if provider_hint.startswith("custom:") and _is_legacy_local_default(bare_model):
            return DEFAULT_MODEL, config_provider, config_base_url
...
def get_effective_default_model(config_data: dict | None = None) -> str:
    ...
    if str(default_model or "").strip().lower() == "qwen3:8b":
        model_cfg = active_cfg.get("model", {})
        base_url = ""
        if isinstance(model_cfg, dict):
        ...
        try:
            ...
            host = ""
        if host in {"api.deepseek.com", "token.kyvolen.com"}:
            default_model = DEFAULT_MODEL
...
def get_available_models() -> dict:
    ...
    def _build_available_models_uncached() -> dict:
        ...
        cfg_base_url = DEFAULT_BASE_URL  # must be defined before conditional blocks (#117)
        model_cfg = cfg.get("model", {})
        if isinstance(model_cfg, str):
            ...
            cfg_base_url = model_cfg.get("base_url", "") or DEFAULT_BASE_URL
            if cfg_default:
                default_model = cfg_default
...
_SETTINGS_DEFAULTS = {
    ...
    "bot_name": os.getenv(
        "HERMES_WEBUI_BOT_NAME", "Cosmius Hermes"
...
def load_settings() -> dict:
    ...
    if settings.get("bot_name") == "Hermes":
        settings["bot_name"] = "Cosmius Hermes"
...
def save_settings(settings: dict) -> dict:
    ...
    def _debug_settings_event(event: str, **fields) -> None:
        try:
            from api.helpers import log_api_event
            log_api_event(event, **fields)
        except Exception:
            logger.debug("Failed to write settings debug event %s", event, exc_info=True)

    _debug_settings_event(
        "settings.persist.start",
        incoming_keys=sorted(str(k) for k in settings.keys()),
        incoming_default_workspace=settings.get("default_workspace"),
        incoming_default_model=settings.get("default_model"),
    )
    ...
    raw_default_workspace = current.get("default_workspace")
    current["default_workspace"] = str(resolve_default_workspace(raw_default_workspace))
    _debug_settings_event(
        "settings.persist.resolved_workspace",
        raw_default_workspace=raw_default_workspace,
        default_workspace=current.get("default_workspace"),
    )
    ...
    _debug_settings_event(
        "settings.persist.written",
        settings_file=str(SETTINGS_FILE),
        default_workspace=current.get("default_workspace"),
        persisted_keys=sorted(str(k) for k in persisted.keys()),
    )
    ...
    _debug_settings_event(
        "settings.persist.ok",
        runtime_default_workspace=str(DEFAULT_WORKSPACE),
        effective_default_model=current.get("default_model"),
    )
    return current
