...
def _agent_state_db_path() -> Path:
    """Return the state.db used by gateway/CLI sessions."""
    raw_config = os.getenv("HERMES_CONFIG_PATH", "").strip()
    if raw_config:
        p = Path(raw_config).expanduser()
        home = p if p.is_dir() else p.parent
...
# ── CLI / gateway session bridge ────────────────────────────────────────────

def _agent_sessions_for_sidebar(*, messaging_only: bool = False) -> list:
    """Read agent-created sessions from state.db in sidebar format."""
    agent_sessions = []
    ...
    try:
        for row in read_importable_agent_session_rows(db_path, limit=200, log=logger, exclude_sources=None):
            if messaging_only and row.get('session_source') != 'messaging':
                continue
            sid = row['id']
        ...
        _logging.getLogger(__name__).warning(
            "agent session listing failed — check state.db schema or path (%s): %s",
            db_path, _cli_err,
        )
        return []
...
def get_cli_sessions() -> list:
    """Read CLI/agent sessions from state.db for optional sidebar display."""
    return _agent_sessions_for_sidebar(messaging_only=False)


def get_gateway_messaging_sessions() -> list:
    """Read gateway messaging sessions that should always appear in the sidebar."""
    return _agent_sessions_for_sidebar(messaging_only=True)
