    ...
    *extra_headers*: optional dict of additional headers to include
    ...
    try:
        ...
        if getattr(exc, "winerror", None) in (10053, 10054):
            return
        if getattr(exc, "errno", None) in (32,):
            return
        raise
...
def _debug_api_logging_enabled() -> bool:
    value = os.environ.get("HERMES_WEBUI_DEBUG_API", "1").strip().lower()
    return value not in {"0", "false", "no", "off"}


def _debug_mask(value: str) -> str:
    if len(value) <= 8:
        return "***"
...
def _debug_redact_value(value, key: str = ""):
    if _DEBUG_SENSITIVE_KEY_RE.search(str(key or "")):
        if isinstance(value, (dict, list)):
            return "[REDACTED]"
        return _debug_mask(str(value or ""))
    if isinstance(value, dict):
        return {str(k): _debug_redact_value(v, str(k)) for k, v in value.items()}
    if isinstance(value, list):
        return [_debug_redact_value(v, key) for v in value[:20]]
    if isinstance(value, str):
        redacted = _DEBUG_SECRET_VALUE_RE.sub(lambda m: _debug_mask(m.group(1)), value)
        return redacted if len(redacted) <= 600 else redacted[:600] + "...[truncated]"
    return value


def log_api_event(event: str, **fields) -> None:
    """Print structured terminal diagnostics during the local testing stage."""
    if not _debug_api_logging_enabled():
        return
    payload = {
        "ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "event": event,
    }
    payload.update(_debug_redact_value(fields))
    try:
        print(f"[webui-debug] {_json.dumps(payload, ensure_ascii=False, default=str)}", flush=True)
    except Exception:
        print(f"[webui-debug] {event} {fields}", flush=True)


def log_api_body(path: str, body: dict) -> None:
    if not _debug_api_logging_enabled() or not isinstance(body, dict):
        return
    if path == "/api/debug/client-event":
        return
    summary = _debug_redact_value(body)
    log_api_event("api.body", path=path, body=summary)
