...
def _debug_workspace_event(event: str, **fields) -> None:
    try:
        from api.helpers import log_api_event
        log_api_event(event, **fields)
    except Exception:
        logger.debug("Failed to write workspace debug event %s", event, exc_info=True)
...
def resolve_trusted_workspace(path: str | Path | None = None) -> Path:
    ...
    """
    if path in (None, ""):
        resolved_default = Path(_BOOT_DEFAULT_WORKSPACE).expanduser().resolve()
        _debug_workspace_event(
            "workspace.resolve.default",
            raw_path=path,
            resolved=str(resolved_default),
            exists=resolved_default.exists(),
            is_dir=resolved_default.is_dir(),
        )
    ...
    if not candidate.exists():
        _debug_workspace_event("workspace.resolve.error", raw_path=str(path), resolved=str(candidate), error="Path does not exist")
        raise ValueError(f"Path does not exist: {candidate}")
    if not candidate.is_dir():
        _debug_workspace_event("workspace.resolve.error", raw_path=str(path), resolved=str(candidate), error="Path is not a directory")
    ...
    if _home != Path("/"):
        try:
            ...
            _debug_workspace_event("workspace.resolve.ok", raw_path=str(path), resolved=str(candidate), reason="home")
            return candidate
        except ValueError:
            pass
    ...
    if _is_blocked_workspace_path(candidate, path):
        _debug_workspace_event("workspace.resolve.error", raw_path=str(path), resolved=str(candidate), error="system directory")
    ...
    try:
        ...
        if candidate in saved_paths:
            _debug_workspace_event("workspace.resolve.ok", raw_path=str(path), resolved=str(candidate), reason="saved")
            return candidate
    except Exception:
        pass
    ...
    try:
        ...
        _debug_workspace_event("workspace.resolve.ok", raw_path=str(path), resolved=str(candidate), reason="boot_default")
        return candidate
    except ValueError:
        pass

    _debug_workspace_event("workspace.resolve.error", raw_path=str(path), resolved=str(candidate), error="outside trusted roots")
    raise ValueError(
