"""
Cosmius Hermes -- Main server entry point.
...
from api.startup import auto_install_agent_deps, fix_credential_permissions, sync_bundled_skills
...
class Handler(BaseHTTPRequestHandler):
    ...
    server_version = ('CosmiusHermesWebUI/' + _ver_suffix) if _ver_suffix != 'unknown' else 'CosmiusHermesWebUI'
...
def main() -> None:
    ...
    if not ok and _HERMES_FOUND:
        print(f'[!!] Warning: Cosmius Hermes agent found but missing modules: {missing}', flush=True)
    ...
    # Apply Cosmius Hermes runtime patches (inject SkillhubChinaSource etc.)
    # Must run after verify_hermes_imports() so hermes-agent is on sys.path.
    try:
        from api.patches import apply_patches
        apply_patches()
    except Exception as e:
        print(f'[!!] WARNING: Cosmius Hermes patches failed to apply: {e}', flush=True)

    STATE_DIR.mkdir(parents=True, exist_ok=True)
    SESSION_DIR.mkdir(parents=True, exist_ok=True)
    DEFAULT_WORKSPACE.mkdir(parents=True, exist_ok=True)
    try:
        from api.logs import cleanup_log_artifacts

        cleanup_log_artifacts()
    except Exception as e:
        print(f'[!!] WARNING: Log cleanup failed: {e}', flush=True)
    ...
    print(f'  Cosmius Hermes listening on {scheme}://{HOST}:{PORT}', flush=True)
