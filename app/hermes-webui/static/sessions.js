// ── Eager exports (run immediately so the left-sidebar functions are reachable
// from messages.js even if anything later in this file ever throws at top
// level).  Function declarations below are hoisted to the top of this script,
// so the assignments resolve to the real functions defined later in the file.
// Without this, a late-declared `window.X = X` block would never run if a
// runtime error fires in between, and the session list would silently die
// with "renderSessionList is not defined" (#left-sidebar-stuck-stale-cache).
if(typeof window!=='undefined'){
  try{
    ...
    window.renderSessionListFromCache=renderSessionListFromCache;
    window.filterSessions=filterSessions;
    if(typeof startGatewaySSE==='function') window.startGatewaySSE=startGatewaySSE;
    if(typeof stopGatewaySSE==='function') window.stopGatewaySSE=stopGatewaySSE;
  }catch(_eagerExportErr){
    if(typeof console!=='undefined' && console && console.warn){
      console.warn('[hermes] sessions.js eager export failed:', _eagerExportErr);
    }
  }
}
...
async function newSession(flash){
  ...
  if(typeof traceComposerSend==='function') traceComposerSend('new_session_start',{workspace:inheritWs||'',model:newModelState.model||''});
  const createSessionWithWorkspace=(workspace)=>api('/api/session/new',{method:'POST',body:JSON.stringify({
  ...
  let data;
  try{
    data=await createSessionWithWorkspace(inheritWs);
  }catch(e){
    const errMsg=String((e&&e.message)||e||'');
    if(typeof traceComposerSend==='function') traceComposerSend('new_session_error',{workspace:inheritWs||'',error:errMsg,status:e&&e.status});
    if(!inheritWs || !/path does not exist|workspace|trusted|invalid/i.test(errMsg)) throw e;
    if(S._profileDefaultWorkspace===inheritWs) S._profileDefaultWorkspace=null;
    if(typeof setComposerStatus==='function') setComposerStatus(`Workspace unavailable, using default workspace: ${errMsg}`);
    else if(typeof setStatus==='function') setStatus(`Workspace unavailable: ${errMsg}`);
    data=await createSessionWithWorkspace(null);
    if(typeof traceComposerSend==='function') traceComposerSend('new_session_retry_default_ok',{failed_workspace:inheritWs||'',session_id:data&&data.session&&data.session.session_id||''});
  }
  S.session=data.session;S.messages=data.session.messages||[];
  if(typeof traceComposerSend==='function') traceComposerSend('new_session_ok',{session_id:S.session.session_id,workspace:S.session.workspace||''});
          ...
          if(Array.isArray(_entries)&&_entries.length){
            ...
            if(_fresh.length){
              ...
              if(_msg&&_first.text&&!_msg.value){
                ...
                if(typeof showToast==='function') showToast((_fresh.length>1?t('queued_messages_restored_multi',_fresh.length):t('queued_message_restored'))+t('queued_review_send_hint'));
              }
...
function _renderBatchActionBar(){
  ...
  archiveBtn.onclick=async()=>{
    ...
    }catch(e){showToast(t('session_toast_archive_failed')+(e.message||e));}
  ...
  deleteBtn.onclick=async()=>{
    ...
    }catch(e){showToast(t('session_toast_delete_failed')+(e.message||e));}
...
function _showBatchProjectPicker(){
  ...
  const none=document.createElement('div');none.className='project-picker-item';none.textContent=t('project_picker_none');
  none.onclick=async()=>{picker.remove();
    try{await Promise.all(ids.map(sid=>api('/api/session/move',{method:'POST',body:JSON.stringify({session_id:sid,project_id:null})})));
      showToast(t('session_toast_removed_from_project'));exitSessionSelectMode();await renderSessionList();
    }catch(e){showToast(t('session_toast_move_failed')+(e.message||e));}
  ...
  for(const p of(_allProjects||[])){
    ...
    item.onclick=async()=>{picker.remove();
      try{await Promise.all(ids.map(sid=>api('/api/session/move',{method:'POST',body:JSON.stringify({session_id:sid,project_id:p.project_id})})));
        showToast(t('session_toast_moved_to_project',p.name));exitSessionSelectMode();await renderSessionList();
      }catch(e){showToast(t('session_toast_move_failed')+(e.message||e));}
...
function _openSessionActionMenu(session, anchorEl){
  ...
  menu.appendChild(_buildSessionAction(
    ...
    session.pinned?SESSION_ICONS.pin:SESSION_ICONS.unpin,
    async()=>{
      closeSessionActionMenu();
  ...
  menu.appendChild(_buildSessionAction(
    ...
    session.archived?SESSION_ICONS.unarchive:SESSION_ICONS.archive,
    async()=>{
      closeSessionActionMenu();
      try{
  ...
  menu.appendChild(_buildSessionAction(
    ...
    async()=>{
      closeSessionActionMenu();
      const oldTitle=(session.title||t('untitled')).trim();
      const nextTitle=window.prompt(t('rename_prompt'), oldTitle);
      if(nextTitle===null) return;
      const trimmed=nextTitle.trim()||t('untitled');
      if(trimmed===oldTitle) return;
      try{
        const res=await api('/api/session/rename',{method:'POST',body:JSON.stringify({session_id:session.session_id,title:trimmed})});
        if(res.session){
          session.title=res.session.title;
          if(S.session&&S.session.session_id===session.session_id) S.session.title=res.session.title;
          const cached=Array.isArray(_allSessions)?_allSessions.find(x=>x.session_id===session.session_id):null;
          if(cached) cached.title=res.session.title;
          renderSessionListFromCache();
          showToast(t('renamed_to')+res.session.title);
        }
  ...
  menu.appendChild(_buildSessionAction(
    ...
    async()=>{
      ...
      try{
        ...
        if(res.session){
          await api('/api/session/rename',{method:'POST',body:JSON.stringify({session_id:res.session.session_id,title:(session.title||t('untitled'))+t('session_duplicate_title_suffix')})});
...
async function probeGatewaySSEStatus(){
  if(_gatewayProbeInFlight) return;
  ...
  try{
    ...
    if(resp.status === 503 || data.watcher_running === false){
      ...
      if(!_gatewaySSEWarningShown && typeof showToast === 'function'){
        showToast(t('gateway_sync_unavailable'), 5000);
...
function _sessionSourceBadgeLabel(session){
  const raw = String((session && (session.raw_source || session.source_tag || session.source)) || '').toLowerCase();
  const isMessaging = (session && session.session_source === 'messaging')
    || ['weixin', 'wechat', 'wx', 'telegram', 'discord', 'slack'].includes(raw);
  if(!isMessaging) return '';
  if(raw === 'weixin' || raw === 'wechat' || raw === 'wx') return t('session_source_weixin');
  const label = String((session && session.source_label) || '').trim();
  return label || '';
}

function _isWeixinSession(session){
  if(!session) return false;
  const raw = String((session.raw_source || session.source_tag || session.source || session.source_label || '')).toLowerCase();
  return ['weixin', 'wechat', 'wx'].includes(raw) || raw.includes('weixin') || raw.includes('wechat');
}

function _sessionSidebarHasMessages(session, activeSidForSidebar){
  if(!session) return false;
  if((session.message_count||0)>0) return true;
  if(session.session_source==='messaging' && _isWeixinSession(session)) return true;
  if(activeSidForSidebar&&session.session_id===activeSidForSidebar) return true;
  if(S.session&&session.session_id===S.session.session_id&&(S.session.message_count||0)>0) return true;
  return false;
}
...
function renderSessionListFromCache(){
  ...
  const withMessages=allMatched.filter(s=>_sessionSidebarHasMessages(s, activeSidForSidebar));
  ...
  const projectFiltered=_activeProject
    ? profileFiltered.filter(s=>s.project_id===_activeProject||s.session_source==='messaging')
    : profileFiltered;
  ...
  if(_sessionSelectMode){
    ...
    exitBtn.textContent='\u2715';exitBtn.title=t('session_exit_select_mode');
  ...
  if(_allProjects.length>0){
    ...
    allChip.textContent=t('session_projects_all');
    ...
    addBtn.textContent='+';
    addBtn.title=t('project_btn_new_title');
  ...
  if(otherProfileCount>0&&!_showAllProfiles){
    ...
    pfToggle.textContent=t('sessions_other_profiles_link',otherProfileCount);
    ...
    const pfToggle=document.createElement('div');
    pfToggle.style.cssText='font-size:10px;padding:4px 10px;color:var(--muted);cursor:pointer;text-align:center;opacity:.7;';
    pfToggle.textContent=t('sessions_active_profile_only_link');
  ...
  if(archivedCount>0){
    ...
    toggle.textContent=_showArchived?t('sessions_archived_hide'):t('sessions_archived_show',archivedCount);
  ...
  if(_activeProject&&sessions.length===0){
    ...
    empty.textContent=t('sessions_empty_in_project');
  ...
  if(pinned.length) groups.push({label:t('session_group_pinned'),items:pinned,isPinned:true});
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    if(cleanTitle.startsWith('[SYSTEM:')){
      cleanTitle=t('session_title_system_placeholder');
    }
    ...
    if(s.pinned&&!isPinnedGroup){
      ...
      pinInd.innerHTML=SESSION_ICONS.pin;
    ...
    const sourceLabel=_sessionSourceBadgeLabel(s);
    ...
    for(const tag of tags){
      ...
      chip.title=t('tag_filter_by_title',tag);
      chip.onclick=(e)=>{
        e.stopPropagation();
    ...
    const startRename=()=>{
      ...
      const finish=async(save)=>{
        ...
        try{
          ...
          applyTitle(oldTitle,false);
          const msg=t('session_rename_failed_prefix')+(err&&err.message?err.message:String(err));
          setStatus(msg);
    ...
    menuBtn.title=t('session_actions_aria');
    menuBtn.setAttribute('aria-haspopup','menu');
    menuBtn.setAttribute('aria-label',t('session_actions_aria'));
    menuBtn.innerHTML=SESSION_ICONS.more;
    menuBtn.onclick=(e)=>{
      e.stopPropagation();
      e.preventDefault();
...
if(typeof window!=='undefined'){
  ...
  window.addEventListener('popstate', () => {
    ...
    if(S.busy){
      if(typeof showToast==='function') showToast(t('session_switch_wait_turn'),3000);
      return;
    }
...
async function deleteSession(sid){
  const ok=await showConfirmDialog({
    message:t('session_delete_confirm_short'),
  ...
  }catch(e){setStatus(t('session_delete_failed_status',e.message));return;}
  if(S.session&&S.session.session_id===sid){
    ...
    if(remaining.sessions&&remaining.sessions.length){
      ...
      const _tt=$('topbarTitle');if(_tt)_tt.textContent=window._botName||'Cosmius Hermes';
      const _tm=$('topbarMeta');if(_tm)_tm.textContent=t('topbar_subtitle_new_chat');
      $('msgInner').innerHTML='';
  ...
  showToast(t('conversation_deleted_toast'));
  await renderSessionList();
}
...
function _showProjectPicker(session, anchorEl){
  ...
  none.onclick=async()=>{
    ...
    renderSessionListFromCache();
    showToast(t('session_toast_removed_from_project'));
  };
  picker.appendChild(none);
  // Project options
  for(const p of _allProjects){
    ...
    item.onclick=async()=>{
      ...
      renderSessionListFromCache();
      showToast(t('session_toast_moved_to_project',p.name));
    };
    picker.appendChild(item);
  }
  ...
  createItem.textContent=t('project_picker_create_new');
  createItem.onclick=async()=>{
    ...
    const name=await showPromptDialog({
      ...
      placeholder:t('project_name_placeholder_short')
    });
    ...
    if(res.project){
      ...
      await renderSessionList();
      showToast(t('project_created_and_moved',res.project.name));
    }
  };
...
function _startProjectCreate(bar, addBtn){
  ...
  inp.placeholder=t('project_name_placeholder_short');
  const finish=async(save)=>{
...
function _startProjectRename(proj, chip){
  ...
  const finish=async(save)=>{
    if(save&&inp.value.trim()&&inp.value.trim()!==proj.name){
      ...
      await renderSessionList();
      showToast(t('project_renamed_toast'));
    }else{
      renderSessionListFromCache();
    }
  };
...
async function _confirmDeleteProject(proj){
  const ok=await showConfirmDialog({
    message:t('project_delete_confirm_message',proj.name),
    confirmLabel:t('delete_title'),
    danger:true
  });
  if(!ok){return;}
  ...
  await renderSessionList();
  showToast(t('project_deleted_toast'));
}
...
if(typeof window!=='undefined'){
  ...
  window.renderSessionListFromCache=renderSessionListFromCache;
  window.filterSessions=filterSessions;
  window.startGatewaySSE=startGatewaySSE;
  window.stopGatewaySSE=stopGatewaySSE;
}
