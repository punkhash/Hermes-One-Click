...
$('btnSend').onclick=()=>{
  if(typeof traceComposerSend==='function') traceComposerSend('btn_send_click',{source:'boot'});
  ...
  if(typeof window.__hermesForceComposerIdle==='function') window.__hermesForceComposerIdle();
  if(typeof send==='function') return send();
};
$('btnAttach').onclick=()=>$('fileInput').click();

function _fallbackSyncSendButton(){
  const btn=$('btnSend');
  const msg=$('msg');
  if(!btn||!msg) return;
  ...
  btn.disabled=!(msg.value||'').trim()&&!(S.pendingFiles&&S.pendingFiles.length);
}

function _bindComposerCoreEvents(){
  const msg=$('msg');
  if(!msg||msg.dataset.coreComposerBound==='1') return;
  msg.dataset.coreComposerBound='1';
  msg.addEventListener('input',()=>{
    if(typeof handleComposerInput==='function') handleComposerInput();
  });
  msg.addEventListener('keydown',e=>{
    if(e.key!=='Enter') return;
    const dd=$('cmdDropdown');
    if(dd&&dd.classList.contains('open')) return;
    const isIme=(typeof window._isImeEnter==='function')
      ? window._isImeEnter(e)
      : (e.isComposing||e.keyCode===229);
    if(isIme) return;
    const mobileDefault=matchMedia('(pointer:coarse)').matches&&window._sendKey==='enter';
    const shouldSend=window._sendKey==='ctrl+enter'||mobileDefault
      ? (e.ctrlKey||e.metaKey)
      : !e.shiftKey;
    if(!shouldSend) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    if(typeof traceComposerSend==='function') traceComposerSend('msg_enter_keydown',{source:'boot',send_key:window._sendKey||'enter'});
    if(typeof window.__hermesForceComposerIdle==='function') window.__hermesForceComposerIdle();
    if(typeof send==='function') send();
    else if(typeof handleComposerPrimaryAction==='function') handleComposerPrimaryAction();
  });
  _fallbackSyncSendButton();
}
_bindComposerCoreEvents();
...
$('msg').addEventListener('paste',e=>{
  ...
  const files=fileItems.map(i=>{
    ...
    return blob.name?blob:new File([blob],blob.name||`file-${Date.now()}`,{type:i.type||'application/octet-stream'});
  }).filter(Boolean);
  if(!files.length) return;
  addFiles(files);
  const hasImage=files.some(f=>f.type.startsWith('image/'));
  setStatus((hasImage?t('image_pasted'):t('file_attached')||'File attached: ')+files.map(f=>f.name).join(', '));
});
...
function applyBotName(){
  ...
  // Falls back to window._botName (global bot_name setting) or 'Cosmius Hermes'.
  let name;
  if(S.activeProfile && S.activeProfile!=='default'){
    name=S.activeProfile.charAt(0).toUpperCase()+S.activeProfile.slice(1);
  }else{
    name=window._botName||'Cosmius Hermes';
  }
  document.title=name;
  const sidebarH1=document.querySelector('.sidebar-header h1');
  if(sidebarH1) sidebarH1.textContent=name;
  const logo=document.querySelector('.sidebar-header .logo');
  if(logo) logo.textContent=name==='Cosmius Hermes'?'CH':name.charAt(0).toUpperCase();
...
(async()=>{
  if(typeof window.__ensureHermesSessions==='function'){
    try{await window.__ensureHermesSessions();}catch(_){}
  }
  ...
  try{
    ...
    window._botName=(s.bot_name&&s.bot_name!=='Hermes')?s.bot_name:'Cosmius Hermes';
    ...
    window._botName='Cosmius Hermes';
    _bootSettings={};
    if(typeof setLocale==='function'){
      const _lang=typeof resolvePreferredLocale==='function'
  ...
  if(typeof initGatewayTitlebar==='function') initGatewayTitlebar();
...
window.addEventListener('pageshow', (event) => {
  ...
  if (typeof refreshGatewayTitlebar === 'function') try { refreshGatewayTitlebar(); } catch (_) {}
});
