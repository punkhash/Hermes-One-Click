...
function traceComposerSend(event, detail={}){
  try{
    const msg=$('msg');
    const payload={
      event,
      ...
      text_len:msg&&msg.value?msg.value.trim().length:0,
      ts:Date.now(),
    };
    console.debug('[hermes-send]', payload);
    fetch(new URL('api/debug/client-event',document.baseURI||location.href).href,{
      method:'POST',
      credentials:'include',
...
window.traceComposerSend=traceComposerSend;

async function _composerNewSession(){
  const fn=window.newSession;
  if(typeof fn==='function') return fn();
  traceComposerSend('new_session_global_missing_fallback');
  ...
  try{
    ...
    traceComposerSend('new_session_fallback_error',{workspace:inheritWs||'',error:errMsg,status:e&&e.status});
  ...
  S.messages=(data.session&&data.session.messages)||[];
  S.lastUsage={...((data.session&&data.session.last_usage)||{})};
  try{localStorage.setItem('hermes-webui-session',S.session.session_id);}catch(_){}
  if(typeof _setActiveSessionUrl==='function') _setActiveSessionUrl(S.session.session_id);
  _markSessionViewed(S.session.session_id,S.session.message_count||0);
  if(S.session.model&&modelSel&&S.session.model!==modelSel.value&&typeof _applyModelToDropdown==='function'){
    _applyModelToDropdown(S.session.model,modelSel,S.session.model_provider||null);
  ...
  traceComposerSend('new_session_fallback_ok',{session_id:S.session.session_id,workspace:S.session.workspace||''});
}

async function _composerRenderSessionList(){
  if(typeof renderSessionList==='function') return renderSessionList();
  if(typeof renderSessionListFromCache==='function') return renderSessionListFromCache();
  traceComposerSend('render_session_list_missing');
}

async function send(){
  if(window._composerSendRunning){
    traceComposerSend('send_duplicate_ignored',{session_id:S.session&&S.session.session_id||''});
    return;
  }
  window._composerSendRunning=true;
  try{
    return await _sendImpl();
  }catch(e){
    const errMsg=String((e&&e.message)||e||'Unknown send error');
    traceComposerSend('send_unhandled_error',{error:errMsg,stack:e&&e.stack?String(e.stack).slice(0,500):''});
    try{removeThinking();}catch(_){}
    try{stopApprovalPolling();}catch(_){}
    try{stopClarifyPolling();}catch(_){}
    if(!S.activeStreamId) setBusy(false);
    setComposerStatus(`Error: ${errMsg}`);
    if(typeof showToast==='function') showToast(`Send failed: ${errMsg}`,3600,'error');
  }finally{
    window._composerSendRunning=false;
    updateSendBtn();
  }
}

async function _sendImpl(){
  const text=$('msg').value.trim();
  traceComposerSend('send_enter',{pending_files:S.pendingFiles.length});
  if(!text&&!S.pendingFiles.length){traceComposerSend('send_return_empty');return;}
  // Don't send while an inline message edit is active
  if(document.querySelector('.msg-edit-area')){traceComposerSend('send_return_editing');return;}
  ...
  if(S.busy||compressionRunning){
    traceComposerSend('send_busy_branch',{compression_running:!!compressionRunning,busy_input_mode:window._busyInputMode||'queue'});
    if(text){
      if(!S.session){traceComposerSend('send_busy_before_new_session');await _composerNewSession();traceComposerSend('send_busy_new_session_ready',{session_id:S.session&&S.session.session_id||'',workspace:S.session&&S.session.workspace||''});await _composerRenderSessionList();}
  ...
  if(text.startsWith('/')&&!S.pendingFiles.length){
    ...
    if(_cmd){
      ...
      if(!_cmd.noEcho){
        if(!S.session){traceComposerSend('send_command_before_new_session');await _composerNewSession();traceComposerSend('send_command_new_session_ready',{session_id:S.session&&S.session.session_id||'',workspace:S.session&&S.session.workspace||''});await _composerRenderSessionList();}
    ...
    if(_parsedCmd&&!_cmd){
      ...
      if(_agentCmd&&_agentCmd.cli_only){
        if(!S.session){traceComposerSend('send_cli_command_before_new_session');await _composerNewSession();traceComposerSend('send_cli_command_new_session_ready',{session_id:S.session&&S.session.session_id||'',workspace:S.session&&S.session.workspace||''});await _composerRenderSessionList();}
        S.messages.push({role:'user',content:text,_ts:Date.now()/1000});
  ...
  if(!S.session){traceComposerSend('send_before_new_session');await _composerNewSession();traceComposerSend('send_new_session_ready',{session_id:S.session&&S.session.session_id||'',workspace:S.session&&S.session.workspace||''});await _composerRenderSessionList();}

  const activeSid=S.session.session_id;

  setComposerStatus(S.pendingFiles&&S.pendingFiles.length?'Uploading…':'');
  let uploaded=[];
  traceComposerSend('send_before_upload',{session_id:activeSid,pending_files:S.pendingFiles.length});
  try{uploaded=await uploadPendingFiles();traceComposerSend('send_upload_done',{session_id:activeSid,uploaded:uploaded.length});}
  catch(e){
    traceComposerSend('send_upload_error',{session_id:activeSid,error:String((e&&e.message)||e||'')});
  ...
  traceComposerSend('send_before_render_user',{session_id:activeSid,workspace:S.session&&S.session.workspace||'',text_len:msgText.length});
  $('msg').value='';autoResize();
  ...
  S.messages.push(userMsg);renderMessages();traceComposerSend('send_user_rendered',{session_id:activeSid,messages:S.messages.length});appendThinking();setBusy(true);
  ...
  if(S.session&&(S.session.title==='Untitled'||!S.session.title)){
    ...
    void _composerRenderSessionList();  // session appears in sidebar immediately
    traceComposerSend('send_title_render_requested',{session_id:activeSid,title:provisionalTitle});
  } else {
    void _composerRenderSessionList();  // ensure it's visible even if already titled
    traceComposerSend('send_sidebar_render_requested',{session_id:activeSid});
  }

  // Start the agent via POST, get a stream_id back
  let streamId;
  try{
    traceComposerSend('send_before_chat_start',{session_id:activeSid,model:S.session.model||$('modelSelect').value,workspace:S.session.workspace});
    ...
    traceComposerSend('send_chat_start_ok',{session_id:activeSid,stream_id:streamId});
    ...
    traceComposerSend('send_chat_start_error',{session_id:activeSid,error:errMsg,status:e&&e.status});
    ...
    const failedAssistantMsg={role:'assistant',content:`**Error:** ${errMsg}`,_error:true,_ts:Date.now()/1000};
    S.messages.push(failedAssistantMsg);
    try{
      const saved=await api('/api/session/record_failed_send',{method:'POST',body:JSON.stringify({
        session_id:activeSid,
        user_message:userMsg,
        error_message:errMsg,
        assistant_message:failedAssistantMsg,
      })});
      if(saved&&saved.session&&S.session&&S.session.session_id===activeSid){
        S.session=saved.session;
        S.messages=saved.session.messages||S.messages;
      }
    }catch(_){/* keep the visible in-memory error even if persistence fails */}
    _queueDrainSid=activeSid;renderMessages();setBusy(false);updateSendBtn();setComposerStatus(`Error: ${errMsg}`);
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function ensureAssistantRow(force=false){
    ...
    const phWait=$('assistantThinkingPlaceholder');if(phWait)phWait.remove();
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('done',e=>{
      ...
      _queueDrainSid=activeSid;void _composerRenderSessionList();setBusy(false);setStatus('');
      setComposerStatus('');
      playNotificationSound();
    ...
    source.addEventListener('apperror',e=>{
      ...
      void _composerRenderSessionList(); // clear streaming indicator immediately on apperror
    });
    ...
    source.addEventListener('cancel',e=>{
      ...
      void _composerRenderSessionList();
      if(!S.session||!INFLIGHT[S.session.session_id]){setBusy(false);setComposerStatus('');}
    });
  }
  ...
  async function _restoreSettledSession(){
    try{
      ...
      _queueDrainSid=activeSid;void _composerRenderSessionList();setBusy(false);setComposerStatus('');
      return true;
    }catch(_){
      return false;
    }
...
function transcript(){
  const lines=[`# Cosmius Hermes session ${S.session?.session_id||''}`,``,
...
function _stashClarifyDraft(reason) {
  ...
  if (reason === "expired" && /^\d{1,4}s?$/i.test(draft)) return false;
...
function sendBrowserNotification(title,body){
  ...
  const botName=window._botName||'Cosmius Hermes';
