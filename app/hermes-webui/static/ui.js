...
window.__hermesForceComposerIdle=function(){
  S.busy=false;
...
function _ensureModelOptionForImmediateSelection(modelId, sel, providerId){
  const value=String(modelId||'').trim();
  if(!value||!sel) return null;
  const applied=_applyModelToDropdown(value,sel,providerId||null);
  if(applied) return applied;
  let group=null;
  const provider=String(providerId||window._activeProvider||'').trim();
  if(provider){
    group=Array.from(sel.querySelectorAll('optgroup')).find(og=>{
      const pid=String((og.dataset&&og.dataset.provider)||'').trim();
      return pid&&pid.toLowerCase()===provider.toLowerCase();
    })||null;
  }
  if(!group){
    group=document.createElement('optgroup');
    group.label=provider?`${provider} (configured)`:'Configured';
    if(provider) group.dataset.provider=provider;
  ...
  if(!opt){
    ...
    opt.title='Configured model';
    group.insertBefore(opt,group.firstChild);
    _dynamicModelLabels[value]=opt.textContent;
  }
  sel.value=value;
  if(sel.id==='modelSelect'&&typeof syncModelChip==='function') syncModelChip();
  return value;
...
// Incremented on each populateModelDropdown() so stale in-flight /api/models
// responses cannot overwrite the dropdown after a later run (e.g. onboarding).
let _populateModelsRequestId=0;

async function populateModelDropdown(options={}){
  const sel=$('modelSelect');
  if(!sel) return;
  const forceDefault=!!(options&&options.forceDefault);
  const preferredModel=String((options&&options.preferredModel)||'').trim();
  const preferredProvider=String((options&&options.preferredProvider)||'').trim();
  const requestId=++_populateModelsRequestId;
  try{
    ...
    if(requestId!==_populateModelsRequestId) return;
    ...
    const modelToApply=preferredModel||data.default_model||'';
    const providerToApply=preferredProvider||data.active_provider||null;
    if(!(data.groups||[]).length && modelToApply){
      const applied=_ensureModelOptionForImmediateSelection(modelToApply,sel,providerToApply);
      if(typeof _writePersistedModelState==='function') _writePersistedModelState(applied||modelToApply,providerToApply);
      else localStorage.setItem('hermes-webui-model',applied||modelToApply);
      if(typeof syncModelChip==='function') syncModelChip();
      return;
    }
    // Set default model from server if no localStorage preference, or when a config
    // flow explicitly asks us to replace stale client state immediately.
    if(modelToApply && (forceDefault || !(typeof _readPersistedModelState==='function'&&_readPersistedModelState()) && !localStorage.getItem('hermes-webui-model'))){
      const applied=_ensureModelOptionForImmediateSelection(modelToApply, sel, providerToApply);
      if(forceDefault){
        const persisted=applied||modelToApply;
        if(typeof _writePersistedModelState==='function') _writePersistedModelState(persisted,providerToApply);
    ...
    if(data.active_provider) _fetchLiveModels(data.active_provider, sel, requestId);
  }catch(e){
...
async function refreshComposerModelFromServer(options={}){
  const preferredModel=String((options&&options.model)||'').trim();
  const preferredProvider=String((options&&options.provider)||'').trim();
  const forceDefault=options&&options.forceDefault!==false;
  ...
  const sel=$('modelSelect');
  const modelToApply=preferredModel||window._defaultModel||((sel&&sel.value)||'');
  const providerToApply=preferredProvider||window._activeProvider||null;
  if(sel&&modelToApply&&typeof _applyModelToDropdown==='function'){
    const applied=_ensureModelOptionForImmediateSelection(modelToApply,sel,providerToApply);
    const persisted=applied||modelToApply;
    if(typeof _writePersistedModelState==='function') _writePersistedModelState(persisted,providerToApply);
    else localStorage.setItem('hermes-webui-model',persisted);
    if(options&&options.updateSession&&S.session){
      S.session.model=persisted;
      S.session.model_provider=providerToApply||null;
      try{
        await api('/api/session/update',{method:'POST',body:JSON.stringify({
          session_id:S.session.session_id,
          workspace:S.session.workspace,
          model:persisted,
          model_provider:providerToApply||null,
        })});
      }catch(e){
        console.warn('session model refresh failed',e);
      }
    }
  }
  if(typeof syncModelChip==='function') syncModelChip();
...
/** Clear client-side live-model cache after provider/config hot-swap (onboarding, etc.). */
function clearLiveModelsClientCache(){
  for(const k of Object.keys(_liveModelCache)) delete _liveModelCache[k];
  // Invalidate any in-flight populateModelDropdown so its response cannot repaint the UI.
  _populateModelsRequestId++;
}
...
function _addLiveModelsToSelect(provider, models, sel){
  ...
  const forcedModel=window._composerForcedModel||null;
  const forcedProvider=window._composerForcedModelProvider||null;
  const currentProvider=forcedProvider||(S.session&&S.session.model_provider)||null;
  ...
  if(forcedModel && sel.id==='modelSelect'){
    const reapplied=_ensureModelOptionForImmediateSelection(forcedModel, sel, forcedProvider||null);
    if(reapplied && typeof syncModelChip==='function') syncModelChip();
  }else if(S.session && S.session.model && sel.id==='modelSelect'){
...
async function _fetchLiveModels(provider, sel, populateRequestId){
  if(!provider||!sel) return;
  const _stale=()=>populateRequestId!=null&&populateRequestId!==_populateModelsRequestId;
...
function syncTopbar(){
  if(!S.session){
    document.title=window._botName||'Cosmius Hermes';
  ...
  document.title=sessionTitle+' \u2014 '+(window._botName||'Cosmius Hermes');
  ...
  const forcedModel=window._composerForcedModel||null;
  const forcedProvider=window._composerForcedModelProvider||null;
  const modelOverride=forcedModel||S._pendingProfileModel;
  let currentModel=S.session.model||'';
  if(modelOverride){
    if(!forcedModel) S._pendingProfileModel=null;
    const providerOverride=forcedProvider||S._pendingProfileModelProvider||null;
    if(!forcedProvider) S._pendingProfileModelProvider=null;
    _ensureModelOptionForImmediateSelection(modelOverride,$('modelSelect'),providerOverride);
    currentModel=modelOverride;
    if(S.session){
      S.session.model=modelOverride;
      S.session.model_provider=providerOverride||S.session.model_provider||null;
    }
  } else {
...
function _assistantRoleHtml(tsTitle=''){
  const _bn=window._botName||'Cosmius Hermes';
  const _isDefaultBot=_bn==='Cosmius Hermes';
  const _avatar=_isDefaultBot
    ? '<div class="role-icon assistant has-image"><img class="role-avatar-img" src="static/logo.png" alt="" decoding="async"></div>'
    : `<div class="role-icon assistant">${esc(_bn.charAt(0).toUpperCase())}</div>`;
  return `<div class="msg-role assistant" ${tsTitle?`title="${esc(tsTitle)}"`:''}>${_avatar}<span style="font-size:12px">${esc(_bn)}</span></div>`;
}
...
function finalizeThinkingCard(){
  ...
  const _phWait=$('assistantThinkingPlaceholder');
  if(_phWait) _phWait.remove();
  if(!isSimplifiedToolCalling()){
...
function appendThinking(text=''){
  // Guard: require an active session only. Do not require S.activeStreamId — sendMessage()
  // calls appendThinking() before /api/chat/start returns stream_id, and loadSession()
  // may call it before S.activeStreamId is assigned on the INFLIGHT reattach path.
  if(!S.session) return;
  ...
  if(!String(text||'').trim()){
    // Simplified mode used to return here — no dots while waiting for first token/reasoning.
    // Match legacy UX: same three-dot row until non-empty thinking arrives.
    let ph=$('assistantThinkingPlaceholder');
    if(!ph){
      ph=document.createElement('div');
      ph.id='assistantThinkingPlaceholder';
      ph.className='assistant-segment';
      ph.setAttribute('data-assistant-waiting','1');
      ph.innerHTML='<div class="thinking"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>';
      blocks.appendChild(ph);
    }
    scrollIfPinned();
    return;
  }
  const phDone=$('assistantThinkingPlaceholder');
  if(phDone) phDone.remove();
  const allChildren=Array.from(blocks.children);
  const anchor=allChildren.filter(el=>
    el.id!=='toolRunningRow' &&
...
function removeThinking(){
  const phRm=$('assistantThinkingPlaceholder');
  if(phRm) phRm.remove();
  if(!isSimplifiedToolCalling()){
