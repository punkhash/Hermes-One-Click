...
let _composerWsPickerParent = null;
...
const APP_TITLEBAR_KEYS = {
  ...
  profiles: 'tab_profiles', todos: 'tab_todos', platforms: 'tab_platforms',
...
async function switchPanel(name, opts = {}) {
  ...
  if (mainEl) {
    ['settings','skills','memory','tasks','workspaces','profiles','insights','platforms'].forEach(p => {
  ...
  if (nextPanel === 'platforms' && typeof loadConnectApps === 'function') {
...
async function openModelProviderSettings() {
  await switchPanel('settings', {bypassSettingsGuard:true});
  switchSettingsSection('providers');
}

window.openModelProviderSettings = openModelProviderSettings;

window.addEventListener('DOMContentLoaded', () => {
  try {
    const params = new URLSearchParams(window.location.search || '');
    if (params.get('open') === 'model-provider') {
      setTimeout(() => { openModelProviderSettings(); }, 300);
    }
  } catch (e) {}
});
...
function toggleComposerWsDropdown(){
  void openComposerWorkspacePicker();
}
...
function _composerWsPickerParentPath(current){
...
function _composerWsPickerRowLabel(fullPath){
  const s=String(fullPath||'');
  const parts=s.split(/[/\\]/).filter(Boolean);
  if(!parts.length) return {leaf:s,parent:''};
  return {leaf:parts[parts.length-1],parent:parts.length>1?parts.slice(0,-1).join('/'):''};
}

function _composerWsSuggestQueryPrefix(path){
  const p=String(path||'').trim();
  if(!p) return '';
  return p.endsWith('/')||p.endsWith('\\')?p:p+'/';
}

function _ensureComposerWorkspacePicker(){
  let modal=$('composerWsPickerModal');
  if(modal) return modal;
  modal=document.createElement('div');
  modal.id='composerWsPickerModal';
  modal.className='workspace-picker-modal';
  modal.setAttribute('role','dialog');
  modal.setAttribute('aria-modal','true');
  modal.innerHTML=`
    <div class="workspace-picker-dialog" onclick="event.stopPropagation()">
      <div class="workspace-picker-head">
        <div>
          <h3>${esc(t('workspace_picker_title'))}</h3>
          <p>${esc(t('workspace_picker_hint'))}</p>
        </div>
        <button type="button" class="workspace-picker-close" id="composerWsPickerClose" aria-label="${esc(t('cancel'))}">×</button>
      </div>
      <div class="workspace-picker-hero">
        <div class="workspace-picker-hero-copy">
          <div class="workspace-picker-kicker">${esc(t('workspace_picker_current_label'))}</div>
          <code id="composerWsPickerActivePath">—</code>
          <h4>${esc(t('workspace_picker_native_title'))}</h4>
          <p>${esc(t('workspace_picker_native_hint'))}</p>
        </div>
        <button type="button" class="workspace-picker-native-btn" id="composerWsPickerNative">
          <span class="workspace-picker-native-icon">${li('folder',16)}</span>
          <span>${esc(t('workspace_picker_native_button'))}</span>
        </button>
      </div>
      <div class="workspace-picker-status" id="composerWsPickerStatus" hidden></div>
      <div class="workspace-picker-section">
        <div class="workspace-picker-section-title">${esc(t('workspace_picker_saved'))}</div>
        <div class="workspace-picker-saved" id="composerWsPickerSaved"></div>
      </div>
      <div class="workspace-picker-fallback-title">${esc(t('workspace_picker_fallback_title'))}</div>
      <div class="workspace-picker-toolbar">
        <code id="composerWsPickerCurrent">—</code>
        <button type="button" class="sm-btn" id="composerWsPickerUp">${esc(t('onboarding_workspace_browse_up'))}</button>
      </div>
      <div class="workspace-picker-list" id="composerWsPickerList"></div>
      <div class="workspace-picker-actions">
        <button type="button" class="sm-btn" id="composerWsPickerCancel">${esc(t('cancel'))}</button>
        <button type="button" class="sm-btn primary" id="composerWsPickerUse">${esc(t('workspace_picker_use'))}</button>
      </div>
    </div>`;
  modal.addEventListener('click',closeComposerWorkspacePicker);
  document.body.appendChild(modal);
  $('composerWsPickerClose').onclick=(e)=>{e.stopPropagation();closeComposerWorkspacePicker();};
  $('composerWsPickerCancel').onclick=(e)=>{e.stopPropagation();closeComposerWorkspacePicker();};
  $('composerWsPickerUp').onclick=(e)=>{e.stopPropagation();void _composerWsPickerGoUp();};
  $('composerWsPickerUse').onclick=(e)=>{e.stopPropagation();void _composerWsPickerUseCurrent();};
  $('composerWsPickerNative').onclick=(e)=>{e.stopPropagation();void _composerWsPickerChooseNative();};
  return modal;
}

function closeComposerWorkspacePicker(){
  const modal=$('composerWsPickerModal');
...
async function openComposerWorkspacePicker(){
  closeWsDropdown();
  closeProfileDropdown();
  if(typeof closeModelDropdown==='function') closeModelDropdown();
  if(typeof closeReasoningDropdown==='function') closeReasoningDropdown();
  const modal=_ensureComposerWorkspacePicker();
  if(!modal) return;
  const chip=$('composerWorkspaceChip');
  const mobileAction=$('composerMobileWorkspaceAction');
  if(chip) chip.classList.add('active');
  if(mobileAction) mobileAction.classList.add('active');
  const current=(S.session&&S.session.workspace)||(typeof S._profileDefaultWorkspace==='string'&&S._profileDefaultWorkspace)||'';
  _composerWsPickerParent=current||null;
  modal.classList.add('open');
  _composerWsPickerSetStatus('');
  _composerWsPickerUpdateActivePath();
  await loadWorkspaceList();
  _renderComposerWsPickerSaved();
  await _composerWsPickerRefresh();
}

function _composerWsPickerActivePath(){
  return (S.session&&S.session.workspace)||(typeof S._profileDefaultWorkspace==='string'&&S._profileDefaultWorkspace)||'';
}

function _composerWsPickerUpdateActivePath(){
  const el=$('composerWsPickerActivePath');
  if(!el) return;
  el.textContent=_composerWsPickerActivePath()||t('no_workspace');
}

function _composerWsPickerSetStatus(message,kind){
  const el=$('composerWsPickerStatus');
  if(!el) return;
  const text=String(message||'').trim();
  el.textContent=text;
  el.hidden=!text;
  el.className='workspace-picker-status'+(kind?` ${kind}`:'');
}
...
function _renderComposerWsPickerSaved(){
  const wrap=$('composerWsPickerSaved');
  if(!wrap) return;
  const list=Array.isArray(_workspaceList)?_workspaceList:[];
  if(!list.length){
    wrap.innerHTML=`<div class="workspace-picker-empty">${esc(t('workspaces_empty_title'))}</div>`;
    return;
  }
  const current=S.session?S.session.workspace:'';
  wrap.innerHTML=list.map(w=>`
    <button type="button" class="workspace-picker-saved-item${w.path===current?' active':''}" data-path="${esc(w.path)}" data-name="${esc(w.name||'')}">
      <span class="workspace-picker-saved-name">${esc(w.name||getWorkspaceFriendlyName(w.path))}</span>
      <span class="workspace-picker-saved-path">${esc(w.path)}</span>
    </button>`).join('');
  wrap.querySelectorAll('.workspace-picker-saved-item').forEach(btn=>{
    btn.onclick=(e)=>{
      e.stopPropagation();
      void _composerWsPickerUsePath(btn.dataset.path||'',btn.dataset.name||'');
    };
  });
}

async function _composerWsPickerGoUp(){
  _composerWsPickerParent=_composerWsPickerParentPath(_composerWsPickerParent||'');
  await _composerWsPickerRefresh();
}

async function _composerWsPickerUseCurrent(){
  const p=String(_composerWsPickerParent||'').trim();
  if(!p){closeComposerWorkspacePicker();return;}
  await _composerWsPickerUsePath(p,'');
}

async function _composerWsPickerUsePath(path,name){
  const target=String(path||'').trim();
  if(!target) return;
  try{
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_picker_use_path_start',{target});
    let displayName=name||'';
    if(!_workspaceList.some(w=>w.path===target)){
      const data=await api('/api/workspaces/add',{method:'POST',body:JSON.stringify({path:target})});
      _workspaceList=data.workspaces||[];
      const added=_workspaceList.find(w=>w.path===target);
      if(added) displayName=added.name||displayName;
    }
    // Keep the blank composer and the next new chat aligned with the user's
    // explicit picker choice.  Without this, a later newSession() can inherit
    // the older profile default and silently jump back to ~/workspace.
    S._profileDefaultWorkspace=target;
    S._profileSwitchWorkspace=target;
    closeComposerWorkspacePicker();
    await switchToWorkspace(target,displayName||getWorkspaceFriendlyName(target));
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_picker_use_path_ok',{target,session_id:S.session&&S.session.session_id||'',session_workspace:S.session&&S.session.workspace||''});
  }catch(e){
    if(String(e.message||'').includes('Workspace already in list')){
      const existing=(_workspaceList||[]).find(w=>w.path===target);
      S._profileDefaultWorkspace=target;
      S._profileSwitchWorkspace=target;
      closeComposerWorkspacePicker();
      await switchToWorkspace(target,(existing&&existing.name)||getWorkspaceFriendlyName(target));
      if(typeof traceComposerSend==='function') traceComposerSend('workspace_picker_use_path_existing_ok',{target,session_id:S.session&&S.session.session_id||'',session_workspace:S.session&&S.session.workspace||''});
      return;
    }
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_picker_use_path_error',{target,error:String((e&&e.message)||e||'')});
    showToast(t('workspace_switch_failed')+(e.message||e));
  }
}

async function _composerWsPickerRefresh(){
  const listEl=$('composerWsPickerList');
  const curEl=$('composerWsPickerCurrent');
  const upBtn=$('composerWsPickerUp');
  const useBtn=$('composerWsPickerUse');
  if(!listEl||!curEl) return;
  const current=_composerWsPickerParent||'';
  curEl.textContent=current||t('onboarding_workspace_browse_roots');
  _composerWsPickerUpdateActivePath();
  if(upBtn) upBtn.disabled=!current;
  ...
  try{
    ...
    listEl.querySelectorAll('.workspace-picker-folder').forEach(btn=>{
      btn.onclick=(e)=>{
        e.stopPropagation();
        _composerWsPickerParent=btn.dataset.path||'';
        void _composerWsPickerRefresh();
      };
    });
  }catch(e){
    listEl.innerHTML=`<div class="workspace-picker-empty">${esc(t('onboarding_workspace_browse_load_failed'))}</div>`;
  }
}
...
async function switchToWorkspace(path,name){
  ...
  if(typeof traceComposerSend==='function') traceComposerSend('workspace_switch_start',{path,session_id:S.session&&S.session.session_id||'',current_workspace:S.session&&S.session.workspace||''});
  const currentIsImportedSession = !!(S.session && (
    S.session.is_cli_session ||
    S.session.session_source === 'messaging' ||
    S.session.source_tag
  ));
  // Opus review Q6: if called from blank page, auto-create a session bound to
  // the requested workspace so the switch doesn't silently no-op. Gateway/CLI
  // sessions are read-only imports from state.db, so switching workspace creates
  // a local WebUI session instead of trying to mutate the imported row.
  if(!S.session || currentIsImportedSession){
    const ws=path||(typeof S._profileDefaultWorkspace==='string'&&S._profileDefaultWorkspace)||'';
    if(!ws){showToast(t('no_workspace'));return;}
    try{
      const r=await api('/api/session/new',{method:'POST',body:JSON.stringify({workspace:ws})});
      if(r&&r.session){S.session=r.session;S.messages=[];S._profileDefaultWorkspace=S.session.workspace||ws;S._profileSwitchWorkspace=S.session.workspace||ws;if(typeof traceComposerSend==='function') traceComposerSend('workspace_switch_created_session',{path:ws,session_id:S.session.session_id,session_workspace:S.session.workspace||'',from_imported_session:currentIsImportedSession});if(typeof syncTopbar==='function')syncTopbar();if(typeof renderMessages==='function')renderMessages();if(typeof renderSessionList==='function')await renderSessionList();}
      if(S.session){
        await api('/api/session/update',{method:'POST',body:JSON.stringify({
          session_id:S.session.session_id, workspace:ws, model:S.session.model, model_provider:S.session.model_provider||null
        })});
        S.session.workspace=ws;
      }
    }catch(e){if(typeof setStatus==='function')setStatus(t('switch_failed')+e.message);return;}
    if(!S.session)return;
    await syncGatewayWorkspaceAfterSwitch(ws);
    showToast(t('workspace_switched_to',name||getWorkspaceFriendlyName(ws)));
    return;
  }
  if(S.busy){
  ...
  try{
    ...
    // Keep both defaults on this choice so the next newSession() cannot fall
    // back to the old profile workspace while the UI is still on the blank page.
    S._profileDefaultWorkspace=path;
    S._profileSwitchWorkspace=path;
    syncTopbar();
    await loadDir('.');
    await syncGatewayWorkspaceAfterSwitch(path);
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_switch_ok',{path,session_id:S.session&&S.session.session_id||'',session_workspace:S.session&&S.session.workspace||''});
    showToast(t('workspace_switched_to',name||getWorkspaceFriendlyName(path)));
  }catch(e){if(typeof traceComposerSend==='function') traceComposerSend('workspace_switch_error',{path,error:String((e&&e.message)||e||'')});setStatus(t('switch_failed')+e.message);}
}

async function syncGatewayWorkspaceAfterSwitch(path){
  try{
    const out=await api('/api/gateway/reload',{method:'POST',body:JSON.stringify({})});
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_gateway_reload_requested',{path,ok:!!(out&&out.ok),status:out&&out.status||''});
  }catch(e){
    if(typeof traceComposerSend==='function') traceComposerSend('workspace_gateway_reload_failed',{path,error:String((e&&e.message)||e||'')});
  }
}
...
async function loadSettingsPanel(){
  try{
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=(settings.bot_name&&settings.bot_name!=='Hermes')?settings.bot_name:'Cosmius Hermes';
...
async function loadProvidersPanel(){
  ...
  try{
    ...
    list.innerHTML='<div style="color:var(--error);padding:12px;font-size:13px">'+esc(t('providers_load_failed', e.message))+'</div>';
  }
}
...
function _buildProviderCard(p){
  ...
  const metaParts=[];
  if(modelCount>0) metaParts.push(t('providers_models_count', modelCount));
  ...
  toggleBtn.textContent=t('providers_key_show');
  toggleBtn.onclick=()=>{
    const revealed=input.type==='text';
    input.type=revealed?'password':'text';
    toggleBtn.textContent=revealed?t('providers_key_show'):t('providers_key_hide');
  };
  ...
  const showBaseUrl=!!p.requires_base_url||!!p.default_base_url||!!p.base_url;
  let baseInput=null;
  if(showBaseUrl){
    ...
    baseInput.type='text';
    baseInput.className='provider-card-input';
    baseInput.value=p.base_url||p.default_base_url||'';
    baseInput.placeholder=p.default_base_url||'https://token.kyvolen.com/v1';
    baseInput.autocomplete='off';
    baseInput.autocapitalize='none';
    baseInput.autocorrect='off';
  ...
  _providerCardEls.set(p.id,{card,input,baseInput,saveBtn,hasKey:p.has_key,baseUrl:p.base_url||p.default_base_url||''});
  const updateSaveState=()=>{
    const keyChanged=!!input.value.trim();
    const baseChanged=!!(baseInput&&baseInput.value.trim()!==String(p.base_url||p.default_base_url||'').trim());
    saveBtn.disabled=!keyChanged&&!baseChanged;
  };
  input.addEventListener('input',updateSaveState);
  if(baseInput) baseInput.addEventListener('input',updateSaveState);
...
async function _saveProviderKey(providerId){
  ...
  const baseUrl=els.baseInput?els.baseInput.value.trim():null;
  const baseChanged=els.baseInput&&baseUrl!==String(els.baseUrl||'').trim();
  if(!key&&!baseChanged){
    showToast(t('providers_enter_key'));
    return;
  }
  if(baseUrl!==null&&baseUrl&&!/^https?:\/\//.test(baseUrl)){
    showToast(t('profile_base_url_rule')||'Base URL must start with http:// or https://');
    return;
  }
  ...
  try{
    ...
    if(baseChanged) payload.base_url=baseUrl||'https://token.kyvolen.com/v1';
    const res=await api('/api/providers',{method:'POST',body:JSON.stringify(payload)});
    if(res.ok){
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=(body.bot_name&&body.bot_name!=='Hermes')?body.bot_name:'Cosmius Hermes';
  if(typeof applyBotName==='function') applyBotName();
...
async function factoryResetAll(){
  const phrase='RESET_ALL_USER_DATA';
  const instructions=(typeof t==='function')?(t('settings_factory_reset_prompt')||''):'';
  const typed=window.prompt(instructions||('Type '+phrase+' to confirm irreversible deletion of all Cosmius Hermes + WebUI data on this machine:'));
  if(!typed||typed.trim()!==phrase)return;
  let password=null;
  try{
    const st=await api('/api/auth/status');
    if(st&&st.auth_enabled){
      password=window.prompt((typeof t==='function')?(t('settings_factory_reset_password_prompt')||''):'Enter your access password to confirm:');
      if(!password)return;
    }
  }catch(e){
    console.warn('[factoryReset] could not read auth status',e);
  }
  const body={confirm:phrase};
  if(password) body.password=password;
  try{
    const r=await api('/api/system/factory-reset',{method:'POST',body:JSON.stringify(body)});
    const msg=(r&&r.message)?r.message:((typeof t==='function')?(t('settings_factory_reset_done')||'Done.'):'Done.');
    if(typeof showToast==='function') showToast(msg);
    window.setTimeout(()=>{ window.location.reload(); },800);
  }catch(e){
    const err=(e&&e.message)?e.message:String(e);
    if(typeof showToast==='function') showToast(((typeof t==='function')?(t('settings_factory_reset_failed')||'Reset failed: '):'Reset failed: ')+err);
    else alert('Reset failed: '+err);
  }
}

async function resetInitialSetup(){
  const ok=await showConfirmDialog({
    title:t('settings_factory_reset_title'),
    message:t('settings_factory_reset_prompt'),
    confirmLabel:t('settings_factory_reset_btn'),
    danger:true,
    focusCancel:true,
  });
  if(!ok) return;
  ...
  try{
    resetStatus=await api('/api/onboarding/reset',{method:'POST',body:JSON.stringify({confirm:true})});
  }catch(e){
    const err=(e&&e.message)?e.message:String(e);
    if(typeof showToast==='function') showToast(((typeof t==='function')?(t('settings_factory_reset_failed')||'Reset failed: '):'Reset failed: ')+err);
    else alert('Reset failed: '+err);
  ...
  try{
    ...
    localStorage.removeItem('hermes-webui-inflight');
    localStorage.removeItem('hermes-webui-inflight-state');
  }catch(_){}
  S.session=null;
  S.messages=[];
  S.toolCalls=[];
  ...
  if(typeof syncTopbar==='function') syncTopbar();
  _settingsDirty=false;
  if(typeof _revertSettingsPreview==='function') _revertSettingsPreview();
  _hideSettingsPanel();
  let wizardShown=false;
  if(typeof loadOnboardingWizard==='function'){
    try{
      wizardShown=await loadOnboardingWizard();
    }catch(e){
      console.warn('load onboarding after reset failed',e);
    }
  }
  if(!wizardShown && resetStatus && resetStatus.completed===false && typeof ONBOARDING!=='undefined'){
    ONBOARDING.status=resetStatus;
    ONBOARDING.active=true;
    ONBOARDING.step=0;
    const onboardingOverlay=$('onboardingOverlay');
    if(onboardingOverlay) onboardingOverlay.style.display='flex';
    if(typeof _renderOnboardingSteps==='function') _renderOnboardingSteps();
    if(typeof _renderOnboardingBody==='function') _renderOnboardingBody();
  }
  if(typeof populateModelDropdown==='function'){
    try{await populateModelDropdown({forceDefault:true});}catch(e){console.warn('populate models after reset failed',e);}
  }
  if(typeof loadWorkspaceList==='function'){
    try{await loadWorkspaceList();}catch(e){console.warn('load workspaces after reset failed',e);}
  }
  if(typeof showToast==='function') showToast(t('settings_factory_reset_done'));
}
...
async function saveSettings(andClose){
  ...
  body.bot_name=(botName&&botName!=='Hermes')?botName:'Cosmius Hermes';
  ...
  if(pw && pw.trim()){
    try{
      ...
      if(body.default_model&&typeof refreshComposerModelFromServer==='function'){
        await refreshComposerModelFromServer({model:body.default_model,forceDefault:true,updateSession:true});
      }
