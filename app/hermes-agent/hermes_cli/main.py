...
def cmd_model(args):
    ...
    if _try_launch_packaged_model_config():
        return
    _require_tty("model")
...
def _try_launch_packaged_model_config() -> bool:
    """Open the bundled Windows WebUI provider settings for installer builds."""
    if os.name != "nt":
        return False
    if os.environ.get("HERMES_DISABLE_PACKAGED_MODEL_GUI") == "1":
        return False

    candidates: list[Path] = []
    for raw in (
        os.environ.get("HERMES_INSTALL_ROOT"),
        os.environ.get("COSMIUS_HERMES_ROOT"),
    ):
        if raw:
            candidates.append(Path(raw))

    exe_path = Path(sys.executable).resolve()
    if exe_path.parent.name.lower() == "python" and exe_path.parent.parent.name.lower() == "runtime":
        candidates.append(exe_path.parent.parent.parent)

    if PROJECT_ROOT.parent.name.lower() == "app":
    ...
    for root in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        launcher = root / "CosmiusHermes.exe"
        ...
        try:
            ...
            print(f"Failed to open Cosmius Hermes model settings: {exc}", file=sys.stderr)
            return False
        print("Opening Cosmius Hermes model provider settings...")
        return True
    return False
